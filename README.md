...
- 固件流水线：采集 → JPEG 编码 → HTTP 发送三个独立阶段
- 模块端缓存：3 MiB 可变长 PSRAM JPEG 环形缓存，吸收短时 Wi-Fi/TCP 抖动
- 优化：ESP32-S3 SIMD、双核 JPEG、DRAM 分块、双帧缓冲、TCP_NODELAY、关闭 Wi-Fi 休眠
- 稳定帧率：限制 GC2145 暗处最长曝光档，优先保证运动流畅度
- 电脑预览：约 4 秒缓存、不抽帧，按摄像头平均帧率匀速播放
...
2. 双击 `启动录像工具.bat`。
3. 填写串口输出的摄像头 IP。
4. 点击“打开摄像头”，程序会自动打开全屏平滑预览，此时不会生成录像文件。
5. 需要录像时点击“开始录制”；停止并保存后预览继续，可再次录制。

录像工具不会二次压缩 JPEG，只把收到的帧封装为 AVI。打开摄像头与开始录制相互独立，但共用同一路图传；开始或停止录像不会重新连接 ESP32。预览完整缓存收到的帧并增加约 4 秒延迟，用更大的缓存换取连续播放。预览断开时不会无限积累旧画面；普通网络抖动不会清空缓存或重新触发 4 秒等待，正常连续图传期间仍不抽帧。

固件端还会持续独立采集和编码，并把编码后的帧写入 3 MiB PSRAM 环形缓存。电脑网络短暂堵塞时，HTTP 发送阶段从缓存继续按顺序读取，不再让网络阻塞直接暂停摄像头；只有堵塞时间长到缓存被完全覆盖时，才会跳到仍保留的最旧帧。
