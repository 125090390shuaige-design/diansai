...
- 预览为 QVGA 320×240、JPEG 质量 80，并限制 GC2145 暗处最长曝光档以优先保证帧率。
- 电脑端缓存约 4 秒，不主动抽帧，按摄像头平均帧率匀速预览。
- 打开摄像头与开始录制是两个独立操作，但共用同一路图传连接；停止录像后预览继续。
- 录像保存为 MJPEG AVI。
...
- 运行 BAT，填写该 IP，先点击“打开摄像头”。
- 等待约 4 秒缓存后应出现全屏预览，此时不应生成录像文件；这段延迟是正常设计。
- 再点击“开始录制”；停止并保存后，预览应继续运行并可再次开始录制。
