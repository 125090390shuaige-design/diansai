...
// ===========================
// iPhone hotspot: use 2.4 GHz / "Maximize Compatibility" mode.
// These values are case-sensitive.
const char* ssid = "nihao";
const char* password = "welcometohere";
...
void setup() {
  ...
  if(config.pixel_format == PIXFORMAT_JPEG){
    ...
    // Two buffers let the camera capture the next frame while the CPU converts
    // the current RGB565 frame. CAMERA_GRAB_LATEST prevents stale-frame delay.
    config.fb_count = 2;
#endif
  }
  ...
  const unsigned long connect_started = millis();
  while (WiFi.status() != WL_CONNECTED && millis() - connect_started < 30000) {
    delay(500);
    Serial.print(".");
  }
  if (WiFi.status() != WL_CONNECTED) {
    Serial.printf("\nWiFi connection failed (status=%d). Restarting...\n", WiFi.status());
    delay(2000);
    ESP.restart();
  }
  Serial.println("");
