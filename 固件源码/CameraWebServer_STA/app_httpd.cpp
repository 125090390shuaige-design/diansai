...
// This firmware is optimized for low-latency streaming. Face processing can
// consume several complete frame times and is deliberately disabled.
...
// esp_new_jpeg uses ESP32-S3 SIMD instructions and block encoding. The small
// aligned block stays in internal RAM while the full camera frame remains in
// PSRAM, which is substantially faster than the legacy fmt2jpg conversion.
static jpeg_enc_handle_t fast_jpeg_handle = NULL;
static uint8_t *fast_jpeg_output = NULL;
static uint8_t *fast_jpeg_block = NULL;
static size_t fast_jpeg_output_capacity = 0;
static int fast_jpeg_block_size = 0;
static int fast_jpeg_width = 0;
static int fast_jpeg_height = 0;

static void fast_jpeg_reset()
{
    if (fast_jpeg_handle) {
        jpeg_enc_close(fast_jpeg_handle);
        fast_jpeg_handle = NULL;
    }
    if (fast_jpeg_output) {
        heap_caps_free(fast_jpeg_output);
        fast_jpeg_output = NULL;
    }
    if (fast_jpeg_block) {
        jpeg_free_align(fast_jpeg_block);
        fast_jpeg_block = NULL;
    }
    fast_jpeg_output_capacity = 0;
    fast_jpeg_block_size = 0;
    fast_jpeg_width = 0;
    fast_jpeg_height = 0;
}

static bool fast_yuv422_to_jpeg(camera_fb_t *fb, uint8_t **out, size_t *out_len)
{
    if (!fb || fb->format != PIXFORMAT_YUV422 || !out || !out_len) {
        return false;
    }

    if (!fast_jpeg_handle || fast_jpeg_width != fb->width || fast_jpeg_height != fb->height) {
        fast_jpeg_reset();
        ...
        if (jpeg_enc_open(&cfg, &fast_jpeg_handle) != JPEG_ERR_OK) {
            log_e("esp_new_jpeg open failed");
            fast_jpeg_reset();
            return false;
        }

        fast_jpeg_block_size = jpeg_enc_get_block_size(fast_jpeg_handle);
        fast_jpeg_output_capacity = (size_t)fb->width * fb->height * 2 + 65536;
        fast_jpeg_output = (uint8_t *)heap_caps_malloc(
            fast_jpeg_output_capacity, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (!fast_jpeg_output) {
            fast_jpeg_output = (uint8_t *)heap_caps_malloc(
                fast_jpeg_output_capacity, MALLOC_CAP_8BIT);
        }
        fast_jpeg_block = (uint8_t *)jpeg_calloc_align(fast_jpeg_block_size, 16);
        if (!fast_jpeg_output || !fast_jpeg_block || fast_jpeg_block_size <= 0) {
            log_e("esp_new_jpeg buffer allocation failed");
            fast_jpeg_reset();
            return false;
        }
        fast_jpeg_width = fb->width;
        fast_jpeg_height = fb->height;
        log_i("esp_new_jpeg ready: %dx%d, block=%d, quality=80",
    ...
    while (offset < fb->len) {
        ...
        if (remaining < (size_t)fast_jpeg_block_size) {
            log_e("Unexpected partial JPEG input block: %u", (unsigned)remaining);
            fast_jpeg_reset();
            return false;
        }
        memcpy(fast_jpeg_block, fb->buf + offset, fast_jpeg_block_size);
        jpeg_error_t err = jpeg_enc_process_with_block(
            fast_jpeg_handle,
            fast_jpeg_block,
            fast_jpeg_block_size,
            fast_jpeg_output,
            fast_jpeg_output_capacity,
            &encoded_size);
        if (err < JPEG_ERR_OK) {
            log_e("esp_new_jpeg process failed: %d", (int)err);
            fast_jpeg_reset();
            return false;
        }
        offset += fast_jpeg_block_size;
    }

    if (encoded_size <= 0) {
        log_e("esp_new_jpeg produced an empty frame");
        return false;
    }
...
static esp_err_t stream_handler(httpd_req_t *req)
{
    ...
    bool _jpg_buf_is_persistent = false;
    char *part_buf[128];
#if CONFIG_ESP_FACE_DETECT_ENABLED
    #if ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_INFO
        bool detected = false;
        ...
        else
        {
            ...
            if (!detection_enabled || fb->width > 400)
            {
                ...
                if (fb->format != PIXFORMAT_JPEG)
                {
                    ...
                    if (fb->format == PIXFORMAT_YUV422) {
                        jpeg_converted = fast_yuv422_to_jpeg(fb, &_jpg_buf, &_jpg_buf_len);
                        _jpg_buf_is_persistent = jpeg_converted;
                    } else {
                        jpeg_converted = frame2jpg(fb, 60, &_jpg_buf, &_jpg_buf_len);
                    }
                    esp_camera_fb_return(fb);
                    fb = NULL;
                    if (!jpeg_converted)
            ...
            else
            {
                if (fb->format == PIXFORMAT_RGB565
                    ...
                    s = fmt2jpg(fb->buf, fb->len, fb->width, fb->height, PIXFORMAT_RGB565, 60, &_jpg_buf, &_jpg_buf_len);
                    esp_camera_fb_return(fb);
                    fb = NULL;
                    if (!s) {
